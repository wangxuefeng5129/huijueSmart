<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-input v-model="listQuery.lightName" placeholder="请输入灯具名称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column property="lightName" align="center" label="灯具名称" sortable />
      <el-table-column property="lampControlName" align="center" label="所属灯控器" sortable />
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="addDialog">
      <el-form ref="form" :rules="rules" status-icon :model="form" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item label="灯具名称" prop="lightName">
          <el-input v-model="form.lightName" placeholder="请输入灯具名称" />
        </el-form-item>
        ...
        <el-form-item label="所属灯制器">
          <el-select v-model="form.lampControlId" placeholder="请选择">
            <el-option v-for="item in lampControlList" :key="item.lampControlId" :label="item.lampControlName" :value="item.lampControlId" />
          </el-select>
        </el-form-item>
      </el-form>
...
<script>
import { fetchList, fetchControlList, fetchLampPostList, addSubmitApi, editSubmitApi, delSubmitApi } from '@/api/light'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        lightName: [{ required: true, message: '请输入灯具名称', trigger: 'blur' }]
      },
  ...
  methods: {
    ...
    handleDelete(scope) { // 删除
      this.$confirm('此操作将永久删除该灯具信息, 是否继续?', '提示', {
        ...
        delSubmitApi(scope.row.lightId).then(res => {
          this.getList()
          this.$notify({
            title: '成功',
            message: '删除成功',
    ...
    handleDownload() { // 导出excel
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['灯具名称', '所属控制器', '所属灯杆']
        const filterVal = ['lightName', 'lampPostId', 'lampPostName']
